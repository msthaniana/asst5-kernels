...
// --- QUAD-BUFFERING: 4 buffers instead of 2 ---
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK) flash_attention_quad_buffered_kernel(
    ...
    // --- Shared Memory Layout (QUAD-BUFFERED for K and V) ---
    ...
    // Helper lambdas for async copies
    ...
    // ========================================================================
    // PROLOGUE: Fill the pipeline (stages 0, 1, 2)
    // ========================================================================
    load_kv_tile(0);  // Start loading iteration 0
    load_kv_tile(1);  // Start loading iteration 1
    load_kv_tile(2);  // Start loading iteration 2

    // ========================================================================
    // MAIN LOOP: 4-stage pipeline
    // Stage 0: Load iteration i+3
    // Stage 1: Wait for iteration i
    // Stage 2: Process K (QK^T) for iteration i
    // Stage 3: Process V (PV) for iteration i
    // ========================================================================
    for (int kv_iter = 0; kv_iter < kv_iters; ++kv_iter) {
        ...
        // --- STAGE 0: Prefetch K and V for iteration (kv_iter + 3) ---
        if (kv_iter + 3 < kv_iters) {
            load_kv_tile(kv_iter + 3);
        }

        // --- STAGE 1: Wait for current iteration's data ---
        // Wait until only (2*NUM_BUFFERS - 2) async groups remain in flight
        // This ensures our current buffer (with 2 groups: K and V) is ready
        asm volatile("cp.async.wait_group %0;" :: "n"(2 * NUM_BUFFERS - 2));
        __syncthreads();

        // --- STAGE 2: Load K and compute S = Q @ K^T ---
        ...
        // --- STAGE 3: Load V and compute O += P @ V ---
...
torch::Tensor flash_attention_forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    ...
    // Shared memory size for QUAD-BUFFERING K and V (4 buffers each)
    const int smem_size = max(BLOCK_ROWS_Q, NUM_BUFFERS * BLOCK_COLS_KV * 2) * HEAD_DIM * sizeof(__half);
    if (smem_size > 48000) {
        cudaFuncSetAttribute(flash_attention_quad_buffered_kernel, 
    ...
    flash_attention_quad_buffered_kernel<<<grid, block, smem_size>>>(
