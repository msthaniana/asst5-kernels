...
constexpr int HEAD_DIM = 128; // Hardcoded as per the problem constraints

// --- Kernel Tuning Parameters ---
constexpr int BLOCK_ROWS_Q = 128;      // How many Q rows a threadblock processes
constexpr int BLOCK_COLS_KV = 32;     // A smaller KV block size enables better pipelining
...
__global__ void __launch_bounds__(THREADS_PER_BLOCK) flash_attention_pipelined_kernel(
    ...
    // --- Shared Memory Layout (with double buffering for K and V) ---
    ...
    for (int kv_iter = 0; kv_iter < kv_iters; ++kv_iter) {
        ...
        // Wait ONLY for K (wait_group 1 means wait until only 1 group remains)
        asm volatile("cp.async.wait_group 1;");
        __syncthreads();

        // Prefetch K for next iteration early
        ...
        // Now wait for V (wait_group 0 means wait for all to complete)
...
torch::Tensor flash_attention_forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    ...
    // Shared memory size for double-buffering K and V.
    const int smem_size = max(BLOCK_ROWS_Q, BLOCK_COLS_KV * 2 * 2) * HEAD_DIM * sizeof(__half);
    if (smem_size > 48000) {
        cudaFuncSetAttribute(flash_attention_pipelined_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
    }

    dim3 grid(div_up(seq_len, BLOCK_ROWS_Q), batch_size * num_heads);
    dim3 block(THREADS_PER_BLOCK);
  
    flash_attention_pipelined_kernel<<<grid, block, smem_size>>>(
