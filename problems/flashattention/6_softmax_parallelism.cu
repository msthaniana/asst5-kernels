...
__global__ void wmma_flash_attention_kernel_v3(
    ...
    for (int j_block_start = 0; j_block_start < seq_len; j_block_start += BLOCK_N) {
        ...
        // === Step 2: Compute Softmax and create P tile (OPTIMIZED) ===
        {
            const int lane_id = threadIdx.x % 32;
            const int warp_id = threadIdx.x / 32;
            const int num_warps = THREADS_PER_BLOCK / 32;  // 4 warps
            ...
            for (int row = warp_id; row < BLOCK_M; row += num_warps) {
                ...
                // === Phase 2: Parallel output rescaling ===
                // All threads in warp cooperate to rescale output
                for (int d = lane_id; d < head_dim; d += 32) {
                    o_tile[row * head_dim + d] *= exp_m_diff;
                }
                
                // === Phase 3: Parallel exp and sum reduction ===
                float l_new = 0.0f;
                
                // Each thread computes exp for its elements
                for (int j = lane_id; j < BLOCK_N; j += 32) {
                    float p_val;
                    if (j_block_start + j < seq_len) {
                        float s_val = s_tile[row * BLOCK_N + j] * scale;
                ...
                // Warp-level reduction to sum exp values
                #pragma unroll
                for (int offset = 16; offset > 0; offset /= 2) {
                ...
                // Only lane 0 updates the statistics
                if (lane_id == 0) {
                    m_tile[row] = m_curr;
                    l_tile[row] = l_curr;
                }
